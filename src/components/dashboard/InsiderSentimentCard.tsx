...
export function InsiderSentimentCard({ transactions }: Props) {
          ...
          {data.topBuyStocks.length > 0 && (
            <div>
              ...
              {data.topBuyStocks.slice(0, 3).map((s, i) => (
                <Link key={i} href={`/stock/${s.ticker}`} className="flex items-center justify-between py-1 text-xs hover:opacity-80">
