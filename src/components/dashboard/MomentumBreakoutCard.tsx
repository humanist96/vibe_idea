...
export function MomentumBreakoutCard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const quotesRes = await fetch("/api/user/portfolio/quotes", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ items: tickerList.map(t => ({ ticker: t, market: "KR" })) }),
        })
        const quotesJson = await quotesRes.json()
        if (!quotesJson.success) return

        const stocks = tickerList.map(t => {
          const q = quotesJson.quotes[t]
          if (!q) return null
          return {
            ticker: t,
            name: q.name ?? t,
    ...
    <div className="rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-card)] p-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-[var(--color-text-primary)]">모멘텀 브레이크아웃</h3>
        </div>
        {data && (
        ...
        <div className="mt-3 space-y-2">
          {data.breakouts.length === 0 ? (
            ...
            data.breakouts.slice(0, 4).map((b, i) => {
              ...
              return (
                <Link key={i} href={`/stock/${b.ticker}`} className="flex items-start gap-2 rounded-lg bg-[var(--color-surface-elevated)] p-2.5 transition-colors hover:bg-[var(--color-surface-elevated)]/80">
