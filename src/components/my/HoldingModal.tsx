  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative w-full max-w-md rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-card)] p-6 shadow-2xl mx-4">
        <div className="mb-5 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1.5 text-[var(--color-text-muted)] hover:bg-[var(--color-surface-100)] transition-colors"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="space-y-4">
          {!isEditing && (
            <>
              ...
              {ticker && (
                <div className="rounded-xl bg-[var(--color-surface-50)] px-4 py-2.5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="10"
              className="w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-50)] px-4 py-2.5 text-sm text-[var(--color-text-primary)] focus:border-amber-400 focus:outline-none focus:ring-1 focus:ring-amber-400"
            />
          </div>
        ...
        <div className="mt-6 flex gap-3">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 rounded-xl border border-[var(--color-border)] px-4 py-2.5 text-sm font-medium text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-50)] transition-colors"
          >
            취소
          </button>
          <button
            ...
            disabled={!canSubmit}
            className="flex-1 rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-amber-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {isEditing ? "수정" : "추가"}
          </button>
        </div>
