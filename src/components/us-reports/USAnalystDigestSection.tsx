...
import type { USStockQuote } from "@/lib/report/us-types"

interface USAnalystDigestSectionProps {
  readonly digest: AnalystDigest
  readonly quote: USStockQuote
}
...
export function USAnalystDigestSection({ digest, quote }: USAnalystDigestSectionProps) {
  const TrendIcon = digest.opinionTrend
    ? TREND_ICONS[digest.opinionTrend as keyof typeof TREND_ICONS]
    : null
  const trendColor = digest.opinionTrend
    ? TREND_COLORS[digest.opinionTrend as keyof typeof TREND_COLORS]
    : ""
      ...
      {digest.targetPriceUpside !== null && (
        <div className="flex items-center gap-3 rounded-lg border border-[var(--color-border-subtle)] p-2.5">
          <div className="text-center">
            ...
            <p
              className={`text-sm font-bold tabular-nums ${digest.targetPriceUpside >= 0 ? "text-green-600" : "text-red-600"}`}
            >
              {digest.targetPriceUpside >= 0 ? "+" : ""}
              {digest.targetPriceUpside}%
            </p>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-xs font-semibold tabular-nums text-[var(--color-text-primary)]">
              {digest.recentReports[0]?.targetPrice
                ? `$${digest.recentReports[0].targetPrice.toFixed(2)}`
                : "-"}
            </p>
          </div>
        </div>
